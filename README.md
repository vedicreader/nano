...
Everything on a dashboard filters together. Pick a column and a value, click a bar, or click a repeated value in a table, and every chart, tile and row on the page narrows to it — `?f=Artist:Name:eq:AC/DC` restates the whole Chinook dashboard around one band. A filter names a column on one table; the charts are drawn from others, so it travels along the foreign keys the schema already declares (Track → Album → Artist, Invoice → InvoiceLine → Track → Album → Artist) as a correlated `EXISTS`. Anything the filter can't reach says it wasn't filtered rather than quietly showing you everything. Filters live entirely in the URL, so a view is a link and the back button undoes it.
