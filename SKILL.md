...
**Every in-dash link goes through `_url(path, fs, **kw)` in `ui.py`** — crumbs, pagers, the tables list, foreign-key cells, column references, the relation tree's htmx URLs. A link that builds `Routes.x.format(...)` by hand silently drops the filter, and a filter that does not survive one click is not a filter. Two links carry *more* than the ambient state: `rel_view`'s "View all N in X" adds the relation that produced the preview (`child.col = val`) so the destination really does show N rows, and `_row_facet` turns the row you are reading into a dashboard filter.
